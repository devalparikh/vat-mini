...
probability  [0.20, 0.66, 0.14]</pre></>,
  ...
  trajectory: {
    title: "Trajectory",
    short: "An ordered record of observations and actions from one episode.",
    detail: <><p>A trajectory preserves what happened first, what the expert did, and what the environment showed next.</p><pre>image₀, action₀, image₁, action₁, …</pre><p>Training uses short windows cut from these records. Their order gives the transformer useful recent history.</p></>,
  },
  cloning: {
    title: "Behavior cloning",
    short: "Learning to reproduce actions demonstrated by an expert.",
    detail: <><p>Each observation and action history is an input. The expert's next action is the target.</p><p>This turns policy learning into supervised classification: predict which of the five actions the expert selected.</p></>,
  },
  batch: {
    title: "Batch",
    short: "A small group of trajectories processed before one weight update.",
    detail: <><p>The model predicts actions for every valid timestep in the batch, averages their losses, and backpropagates that combined error.</p><p>An update is the moment the optimizer changes the model's internal weights. One batch produces one update.</p><pre>predict → measure error → backpropagate → change weights once</pre><p>In the smoke run, a batch contains 4 trajectories. With 128 training trajectories, the model performs 32 updates per epoch.</p></>,
  },
  epoch: {
    title: "Epoch",
    short: "One complete pass through the training dataset.",
    detail: <><p>After one epoch, every training trajectory has contributed to learning once. The same dataset is reused in later epochs, but its order is reshuffled and the model begins each pass with updated weights.</p><p>Repeated passes let the model make progressively smaller corrections. Validation data helps reveal when training improves memorization without improving performance on unseen examples.</p></>,
  },
  advantage: {
    title: "Advantage weighting",
    short: "Giving more influence to demonstrated actions linked to better outcomes.",
    detail: <><p>Ordinary behavior cloning treats each demonstrated action equally. Advantage weighting asks which actions led to better results and makes those examples count more.</p><p>The model still predicts the expert action. The weight only changes how strongly that example affects learning.</p></>,
  },
};
...
const lessonIndex = [
  ...
  ["evaluation", "07", "Evaluation", "How to test the policy in closed loop"],
] as const;
...
export default function ArchitecturePage() {
  return <main className="learn-page page-with-editorial-toc">
    ...
    <section className="learn-section token-section" id="token"><div className="learn-heading"><span>03 / TIMESTEP TOKEN</span><h2>Visual, action, and position vectors are combined.</h2><p>The result is one <Term id="token">timestep token</Term> that represents the current moment.</p></div><TokenMath/><div className="explain-pair"><article><span>ORDERED EXPERIENCE</span><h3>Tokens come from a <Term id="trajectory">trajectory</Term>.</h3><p>A trajectory records images and expert actions in time order. Training takes a few neighboring moments at a time. This short context window gives the model recent history without requiring the entire episode.</p></article><article><span>SEQUENCE SHAPE</span><h3><code>[B, T, D]</code> is a compact description of the token data.</h3><p><code>B</code> means how many trajectory windows are processed together. <code>T</code> means moments in each window. <code>D</code> means numbers in each token. For example, <code>[3, 4, 96]</code> holds 3 windows, with 4 moments represented by 96 numbers each.</p></article></div></section>

    <section className="learn-section attention-section" id="attention"><div className="learn-heading"><span>04 / ATTENTION</span><h2>Each timestep uses current and earlier information.</h2><p><Term id="attention">Attention</Term> connects the selected timestep to relevant earlier timesteps. It cannot use future information.</p></div><AttentionLab/><div className="qkv-strip"><div><b>Query</b><span>What information do I need?</span></div><div><b>Key</b><span>What kind of information do I contain?</span></div><div><b>Value</b><span>What information should I send?</span></div></div><div className="explain-pair"><article><span>MULTIPLE HEADS</span><h3>The model can compare the same history in several ways.</h3><p>These parallel comparisons are called attention heads. One head might focus on recent movement while another focuses on an earlier view of the target. The model combines what they find.</p></article><article><span>CONTEXT COST</span><h3>A longer memory requires more comparisons.</h3><p>With 4 moments, there are up to 16 moment-to-moment comparisons. With 8, there are up to 64. This is why attention cost grows with the square of the context length: <code>T²</code>.</p></article></div></section>
    ...
    <section className="learn-section learning-section" id="training"><div className="learn-heading"><span>06 / TRAINING</span><h2>Action errors update the entire model.</h2><p><Term id="cloning">Behavior cloning</Term> compares the policy's prediction with the expert's action. That one action <Term id="loss">loss</Term> trains the vision encoder, transformer, and action head together.</p></div><TrainingFlow/><div className="explain-pair"><article><span>BATCH</span><h3>A small group of trajectories produces one weight update.</h3><p>The model predicts actions for one <Term id="batch">batch</Term>, averages what it got wrong, backpropagates the correction, and changes its internal weights once. That weight change is one update. In the smoke run, 4 trajectories make one batch, so 128 trajectories produce 32 updates per epoch.</p></article><article><span>EPOCH</span><h3>One epoch is one pass through all training data.</h3><p>Later <Term id="epoch">epochs</Term> reuse the same trajectories in a reshuffled order. The data is the same, but the model is different after every update, so each pass corrects the mistakes that remain. Eight smoke epochs produce 256 updates.</p></article></div><div className="explain-pair"><article><span>REAL-WORLD FINE-TUNING</span><h3>A large model may fit only a few examples on each GPU.</h3><p>OpenVLA-OFT used 4 examples per GPU across 8 GPUs: a global batch of 32. Runs lasted 50,000–150,000 updates because multi-camera images and billions of model parameters make each example expensive.</p></article><article><span>LARGE-SCALE PRETRAINING</span><h3>Many devices combine their examples into one global batch.</h3><p>Octo-Base used a global batch of 2,048 for 300,000 updates on 800,000 robot trajectories. Its training was distributed across 128 TPU chips; no single device held the entire batch.</p></article></div><p className="training-takeaway"><strong>Effective global batch</strong> = examples per device × devices × accumulated passes. Large runs are usually reported in optimizer steps rather than epochs because they mix datasets, resample trajectory windows, and change examples with augmentation.</p><div className="training-contrast"><article><span>PRETRAINING</span><h3>Learn the general behavior first.</h3><p>Broad expert trajectories teach the model how images, recent actions, and useful movement patterns relate. Each demonstrated action contributes equally to the lesson.</p></article><article><span>POST-TRAINING</span><h3>Focus learning on better outcomes.</h3><p>A smaller set of task-focused examples can refine the policy. <Term id="advantage">Advantage weighting</Term> makes actions linked to better results count more. Smaller weight updates help preserve what the model already learned.</p></article></div><div className="explain-pair"><article><span>WHY REPEAT THE DATA?</span><h3>One pass usually leaves useful corrections unfinished.</h3><p>Each batch only nudges the weights. Repeated passes reinforce common patterns and improve harder cases. Training loss should generally fall as predictions improve.</p></article><article><span>WHEN TO STOP</span><h3>Validation checks whether improvement transfers.</h3><p>If training improves while performance on unseen trajectories worsens, the model may be memorizing the training set. More epochs are useful only while learning continues to generalize.</p></article></div></section>

    <section className="learn-section evaluation-section" id="evaluation"><div className="learn-heading"><span>07 / EVALUATION</span><h2>Test the policy on the histories it creates itself.</h2><p>Training asks whether the model can copy the next expert action. Evaluation must also ask whether those predictions produce successful behavior.</p></div><div className="training-contrast"><article><span>UNSEEN EXPERT TRAJECTORIES</span><h3>First, check prediction on examples that were not used for training.</h3><p>The history still contains expert actions. Loss and accuracy reveal whether the model learned a reusable pattern instead of only remembering its training examples.</p></article><article><span>CLOSED-LOOP ROLLOUT</span><h3>Then, let the policy create its own next observation.</h3><p>The policy acts, receives the resulting image, and acts again until the episode ends. One mistake may create an unfamiliar situation, so this test also reveals whether the policy can recover.</p></article></div><div className="explain-pair"><article><span>FAIR COMPARISON</span><h3>Start each saved model from the same situations.</h3><p>A fixed seed recreates the same starting position and environment choices. The resulting rollouts are easier to compare because the challenge stayed the same.</p></article><article><span>WHAT COUNTS</span><h3>Lower loss is useful evidence—not the final result.</h3><p>A model can predict expert actions well and still fail after its own first mistake. Higher task completion during rollout is stronger evidence that the policy actually improved.</p></article></div></section>
